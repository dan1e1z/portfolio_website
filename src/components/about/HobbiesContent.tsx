...
const HobbiesContent = ({ containerRef }: HobbiesContentProps) => {
  ...
  const textSize = useMemo(() => {
    return dimensions?.width !== undefined && dimensions.width < 615
      ? "text-xl"
      : "text-2xl";
  }, [dimensions?.width]);
  ...
  const squares = useMemo(() => {
    return colors.map((color, index) => {
      ...
      return {
        color,
        yRange: [start, end],
        opacityRange: [start, Math.min(start + 0.1, 0.95)],
      };
    });
  }, []);
      ...
      >
        <div className="grid w-full h-full grid-cols-8 grid-rows-3 gap-2">
          {squares.map((square, index) => (
            <motion.div
              key={index + 1}
              className={`aspect-square w-full h-full ${square.color} ${positions[index + 1]}`}
              style={{
                y: useTransform(
                  scrollYProgress,
                  square.yRange,
